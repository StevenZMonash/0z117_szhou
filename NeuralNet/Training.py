...
def load_data():
    ...
    data = []
    for i in range(5, 99):
        with open(f'../data_gen/out/p_a1_given_q_xeff_qid_{i}.pkl', 'rb') as f:
            temp = pickle.load(f)
...
def get_model(q):
    ...
    Input2 = Input(shape=(len(q[0]),))
    Input3 = Input(shape=(len(q[0], )))
...
def evaluate(model,test_q,test_xeff,test_a1,test_p,history):
    outdir = 'out'
    os.makedirs(outdir, exist_ok=True)
    model.evaluate([test_q, test_xeff, test_a1],test_p)
    # plot history loss
    ...
    for i in range(len(predict)):
        if i in np.random.randint(len(predict),size=10): # plot 10 random distribution
            plt.clf()
            plt.plot(test_a1[i], test_p[i],
                     linestyle='dashed', alpha = 0.5)
            plt.plot(test_a1[i], predict[i],
                     linestyle='dotted', alpha = 0.5)
            plt.title(f'prediction vs true p_a1 given q = {test_q[i,0]}, xeff={test_xeff[i,0]}')
            plt.legend(['actual','predicted'],bbox_to_anchor=(1.05, 1))
            plt.xlabel('a1')
            plt.ylabel('p')
            plt.savefig(f'out/q = {test_q[i,0]}, xeff={test_xeff[i,0]}.png')
...
def main():
    ...
    history = model.fit([train_q, train_xeff, train_a1], train_p,
                        epochs=500, batch_size=32, verbose=2, validation_split=0.1)
    model.save("my_model")
    evaluate(model,test_q,test_xeff,test_a1,test_p,history)
